...
**Last updated:** September 25, 2026 | **Repo:** private | **Owner:** Sarah Evenson
...
**Corrected Sept 5, 2026 — this whole section was wrong.** Edge functions DO auto-deploy: `.github/workflows/deploy-edge-functions.yml` runs on every push to `main` that touches `supabase/functions/**`, deploys every function via `supabase functions deploy`, then explicitly re-deploys the functions that must NOT require a Tandril login token (`--no-verify-jwt`) — the ones called directly by Shopify/eBay/Stripe rather than by a logged-in user's browser. This has been running successfully for a long time (141+ runs going back months) — manual dashboard pasting was never actually required once this workflow existed; the note saying otherwise was just stale and every session since then repeated it without checking.

**How to deploy a function from a Claude Code session directly (no PR merge needed, e.g. to test a fix live before merging):** the Supabase CLI works from here now — it's installed via `npm install -g supabase`, logged in via `supabase login --token $SUPABASE_ACCESS_TOKEN`, and linked via `supabase link --project-ref biksocozipayckfuzzul`. This requires the Claude Code cloud environment's **Network access** to be set to **Custom** with `api.supabase.com` and `*.supabase.co` allowed, and a `SUPABASE_ACCESS_TOKEN` environment variable set on that same environment (Sarah generated this token from Supabase Dashboard → Account → Access Tokens once, Sept 5 2026 — a session without that environment configured will hit `request blocked: no rule or allowlist entry allows host "api.supabase.com"` on `supabase link`). Once linked, deploy with `supabase functions deploy <name> --project-ref biksocozipayckfuzzul` — add `--no-verify-jwt` for any function on the list above, or the live function will start requiring a login token it shouldn't (see next paragraph for exactly this happening).

**Real bug found and fixed Sept 5, 2026 while first testing this pipeline:** `shopify-order-webhook` was missing from the workflow's `--no-verify-jwt` list — confirmed live via `supabase functions list` that it had drifted to `verify_jwt: true`, which means Supabase's own gateway would reject every real Shopify webhook call (order paid, cancelled, refund) with a 401 before the function's own signature check ever ran, regardless of any code-level fix. Fixed live via CLI and added the missing step to the workflow so a future automatic deploy can't silently reintroduce this. If cross-platform inventory sync ever stops reacting to real Shopify orders again, check this setting first (`supabase functions list --project-ref biksocozipayckfuzzul`, look for `shopify-order-webhook`'s `verify_jwt` — it must be `false`).
...
- **`ebay-notification-webhook` missing from the deploy pipeline's `--no-verify-jwt` list — same bug class as the `shopify-order-webhook` fix from Sept 5, never caught for eBay (found and fixed Sept 25, 2026):** eBay's real-time order webhook (`ebay-notification-webhook`, built May 8, 2026 — handles eBay's GET challenge-response handshake and POST order notifications, then fires `sync-inventory-levels`) was never added to `.github/workflows/deploy-edge-functions.yml`'s list of functions deployed with `--no-verify-jwt`. Confirmed live: a direct GET request to the function's public URL (the same request eBay itself sends to activate the notification subscription) returned a bare `401` from Supabase's gateway, before the function's own code ever ran — meaning eBay's challenge verification would fail, the notification subscription registered in `ebay-auth-callback` would never actually activate, and even if it somehow did, every real order notification would also be rejected with 401. **Net effect: an eBay sale has not been auto-syncing to other platforms** — sellers have been relying on the manual "Sync Now" button in the Inventory page's Cross-Platform Sync Links panel, same as a manual edit. Fixed by (1) deploying `ebay-notification-webhook` directly via `supabase functions deploy ebay-notification-webhook --no-verify-jwt` (Supabase confirms `verify_jwt: false`, version 32, ACTIVE — this is the immediate live fix), and (2) adding the missing `--no-verify-jwt` step to the GitHub Actions workflow so a future push to `main` can't silently regress it back to `verify_jwt: true` the way the Shopify one did before Sept 5. **Not yet independently confirmed with a real live eBay test order** (the Sept 19 Shopify test caught a second, unrelated bug that only a real order surfaced) — worth doing the same kind of live test for eBay before fully trusting instant eBay sync. **If eBay sales ever stop reflecting elsewhere again, check this setting first** (`supabase functions list --project-ref biksocozipayckfuzzul`, look for `ebay-notification-webhook`'s `verify_jwt` — it must be `false`) before assuming the sync logic itself is broken.
...
- **eBay Sell Inventory API calls were missing required headers — fixed everywhere they occur (Aug 29, 2026):** Browse & Link's eBay column returned "Could not load products: eBay inventory fetch failed: 400" with the actual eBay error body silently discarded (`fetch-platform-products`'s `fetchEbayProducts` only threw the HTTP status, never `res.text()`). Root cause: eBay's Sell Inventory API (`/sell/inventory/v1/inventory_item`, used for the product browse list, `link-products`' eBay scan, and `sync-inventory-levels`' eBay read/write) requires a `Content-Language` header on every call — including GETs — and expects `X-EBAY-C-MARKETPLACE-ID` explicitly; `fetch-platform-products` had neither, `link-products` had only the marketplace header, `sync-inventory-levels` had neither. Fixed all three call sites in `/home/user/Tandril/supabase/functions/fetch-platform-products/index.ts`, `/home/user/Tandril/supabase/functions/link-products/index.ts`, and `/home/user/Tandril/supabase/functions/sync-inventory-levels/index.ts` (added a shared `headers` object out of `resolveEbayToken()` there) — all now send `Content-Language: en-US` + `X-EBAY-C-MARKETPLACE-ID: <marketplace_id or EBAY_US>`, and all now capture and surface the real eBay error body in the thrown `Error` instead of just the status code, so a future failure shows the actual reason instead of a bare `400`. **Separately found and fixed while investigating:** the eBay username showing as the literal placeholder `"eBay User"` instead of the real seller name (confirmed live via edge logs: `GET .../commerce/identity/v1/user/` returned `404` with an empty body) was caused by `ebay-auth-callback/index.ts` calling eBay's Commerce Identity API on the wrong subdomain — `api.ebay.com` instead of `apiz.ebay.com` (eBay hosts Commerce Identity separately from every other Sell/Commerce API this file talks to, which do correctly use `api.ebay.com`). Fixed by adding a dedicated `identityApiBaseUrl` for that one call. **Only takes effect on a fresh connect** — same caveat as the other "metadata added after the fact" eBay fixes: an already-connected eBay platform needs disconnect/reconnect to pick up the real username. All five touched functions (`fetch-platform-products`, `link-products`, `sync-inventory-levels`, `fetch-product-variants`, `ebay-auth-callback`) were manually deployed to Supabase same-day.

  **Follow-up same day:** the `Content-Language: en-US` fix above got further — the 400 changed to a more specific eBay error, `{"errorId":25709,...,"message":"Invalid value for header Accept-Language."}`. eBay's Sell Inventory API validates `Accept-Language` (not `Content-Language`) on GET/read calls — `Content-Language` is for calls carrying a request body (the PUT in `syncEbay`). All three eBay call sites (`fetch-platform-products`, `link-products`, `sync-inventory-levels`'s shared `resolveEbayToken()` headers) now send **both** `Content-Language: en-US` and `Accept-Language: en-US` on every call rather than splitting them per method, since eBay tolerates the extra header and this avoids missing a spot. Redeployed same day.

  **Confirmed working Aug 30, 2026, but surfaced a real eBay data-model gotcha, not a Tandril bug — read before troubleshooting "stale" eBay listings again.** After the Accept-Language fix, Sarah reconnected eBay (new platform row `076b0eef...`, real username `saraevens` now resolving correctly — the `apiz.ebay.com` identity fix confirmed live) and reported Browse & Link's eBay column showing a deleted listing ("Still Vesting T-Shirt S", SKU `1218588_4046`) and missing a newly-added one (a Halloween tote). Investigated for a caching bug and found none: `fetch-platform-products` has no cache table, no in-memory cache, and does a genuine live `GET` to eBay on every call — confirmed via edge logs showing 15+ separate `fetch-platform-products` invocations across her testing window, every one a fresh `200`. Also confirmed only one eBay `platforms` row exists (no stale duplicate row being read instead of the new one) and eBay is the one platform type never written to the local `products` table (that upsert path is Instagram-only), so there's no local table this could be reading from either. **Root cause is eBay's own API, not Tandril:** the Sell Inventory API's `inventory_item` object is a separate data model from a marketplace listing/offer — ending or deleting a listing in Seller Hub does not delete the underlying `inventory_item` record (that needs an explicit `DELETE /sell/inventory/v1/inventory_item/{sku}`, which nothing in this codebase calls), so a "deleted" SKU can keep appearing indefinitely. Conversely, a listing created through eBay's classic/quick-listing flow without going through SKU-based "manage inventory" tracking may never get an `inventory_item` record created at all, so it won't appear via this endpoint no matter how fresh the fetch is. **Fix is on the eBay side, not in code:** to actually remove a stale SKU, delete the inventory item itself (Seller Hub → Inventory, not just the listing); to make a new listing appear, confirm it has a SKU assigned under eBay's inventory-tracked flow, not a quick "sell similar" listing. For the Sept 8 demo, verify both directly in eBay Seller Hub before assuming a Tandril refresh will fix it — no refresh will, because Tandril is already showing eBay's own live answer.
- **Product Linker SKU auto-fill was pinned to the wrong variant — fixed (Aug 29, 2026):** in `components/inventory/ProductLinkerModal.jsx`, picking a product auto-filled the SKU field once, from the *first* variant only (Shopify's `fetchShopifyProducts` sets a product's top-level `sku` to `variants[0]?.sku`) — and then never updated again, even after selecting a different variant, because the auto-fill `useEffect` didn't depend on the variant selection at all and self-disabled after the first non-empty value. So linking a specific size/variant other than the first one silently used the wrong SKU. Fixed in two places: `supabase/functions/fetch-product-variants/index.ts` now returns each variant's own `sku` (previously only `id`/`label`, with the SKU baked into the display label as text and nowhere else); `ProductLinkerModal.jsx` now recomputes the SKU from the *selected* variant (falling back to the product-level SKU only when no variant is chosen) and reacts to variant changes, while a `skuAutoFilled` flag stops it from clobbering a SKU the user typed by hand until the next product/variant pick or modal reset.
...
- **Shopify webhook auth, GDPR deletion bug, and Faire credential mismatch — all fixed (Sept 5, 2026, PR #170), plus the deploy pipeline itself corrected:** three bugs found in a full platform-integration audit that same day. (1) `shopify-order-webhook`'s signature check only ran if a per-shop `metadata.webhook_secret` existed on the `platforms` row — nothing ever set that secret, so verification silently never ran; now verifies against `SHOPIFY_API_SECRET` (the app's own shared secret, already correctly used by the billing webhooks) before touching the database. (2) `customers-redact`/`shop-redact` deleted rows by filtering `ai_commands`/`saved_commands`/`ai_workflows`/`workflow_runs` on a `shop_domain` column those tables don't have, so those deletions silently failed every time — now resolves the owning user via the `platforms` table first, then deletes by `user_id`; also deleted the unused draft rewrites (`shopify-customers-redact`, `shopify-shop-redact`, `shopify-customers-data-request`) since their corrected logic now lives in the real deployed functions (those draft files themselves had a separate bug — logging to a `gdpr_requests` table that was never created, which would have crashed the whole webhook). (3) Faire's manual-token connect saved the credential as `credentials.api_token`, but every read/write action and the OAuth connect path looked up `credentials.access_token` — renamed to match; also fixed the identical mismatch in `ai-coach-chat`'s Faire branch, found while checking other call sites. **Existing Faire connections made before this fix need to disconnect/reconnect.**
  - **Also corrected the "manual dashboard paste only" deployment model documented above** — it was stale. `.github/workflows/deploy-edge-functions.yml` already auto-deploys on merge to main and has for months (141+ successful runs). See the rewritten "Supabase Edge Functions" section above for the real deployment model, including how to deploy directly from a Claude Code session via the Supabase CLI, and a real `verify_jwt` regression found and fixed on `shopify-order-webhook` while first testing that pipeline end-to-end.
- **Walmart Marketplace — same "0 products on Products/Inventory page" bug class as WooCommerce/BigCommerce/Instagram, found and fixed proactively (Sept 20, 2026, code fixed, NOT yet live-tested):** Walmart's connect flow (`WalmartConnectButton.jsx`, `walmart-connect` edge function, self-service `client_id`+`client_secret` — no OAuth review needed, same category as WooCommerce/BigCommerce), Orion chat awareness (`smart-api`'s `getUserStoreContext` walmart branch), and all five write actions (`walmart_update_inventory`, `walmart_update_price`, `walmart_update_title`, `walmart_update_description`, `walmart_end_listing`) were already fully built — but the Products and Inventory pages would have shown 0 (or a permanently stale) product list for any connected Walmart store, for the same reason as the earlier platforms: `smart-api`'s `get_inventory` action (what both `pages/Products.jsx` and `pages/Inventory.jsx` actually call for live data) had no Walmart branch at all, and `Products.jsx`'s `LIVE_FETCH_PLATFORMS` didn't include `'walmart'` either — so the Products page fell back to the local `products` table, which only ever got a one-time snapshot from `walmart-connect`'s initial sync at connect time and was never refreshed again. Fixed by adding a live Walmart fetch (`GET /v3/items`, same token-exchange pattern already used by `walmart-connect` and the write actions) to `get_inventory`, and adding `'walmart'` to `LIVE_FETCH_PLATFORMS`. **Also fixed in passing:** `walmart-connect` never set `shop_name` on the `platforms` row (only `name` and `metadata.seller_name`), so `PlatformCard.jsx` showed a bare "Connected" badge with no seller name underneath — the same gap already fixed for eBay (PR #151) and BigCommerce (PR #160). **Existing Walmart connections need to disconnect/reconnect** to pick up `shop_name`. **Not yet verified against a real Walmart Seller Center account** — no test credentials available this session; before telling a merchant this works end-to-end, get real Walmart Marketplace API credentials (Seller Center → Settings → Consumer IDs & Private Keys) and do a real connect-and-view-products test the way WooCommerce/BigCommerce were verified on Aug 8.
- **Products/Inventory live fetch — last five platforms added, duplicate BigCommerce fetch removed (Sept 25, 2026, code fixed, NOT yet live-tested):** audited `smart-api`'s `get_inventory` action (what `pages/Products.jsx` and `pages/Inventory.jsx` call) against every connect button. Two problems: (1) **BigCommerce was fetched twice** — a second, older block further down re-fetched the same store, so every BigCommerce product showed up **twice** on the Products/Inventory pages, and the duplicate copy read a field BigCommerce doesn't have (`inventory_quantity` instead of `inventory_level`), so the duplicate always showed 0 stock / "out of stock". Removed the duplicate; the earlier, correct, paginated block stays. (2) **Ecwid, Magento, PrestaShop, Wish and Etsy had no live fetch at all** — same "0 products" bug class as WooCommerce/BigCommerce/Instagram/Walmart. Their write actions and Orion chat awareness (`getUserStoreContext`) already existed; only the Products/Inventory pages were blind. Added paginated live fetches for all five (PrestaShop also reads product-level stock from `/api/stock_availables`, since `/api/products` has no quantity; Ecwid products without stock tracking are treated as in stock, not out of stock). Also added all five plus `faire`/`square`/`wix`/`squarespace` (which already had `get_inventory` branches) to `Products.jsx`'s `LIVE_FETCH_PLATFORMS`. None of the five new fetches have been tested against a real store.
- **Etsy access-token refresh added everywhere (Sept 25, 2026, same PR, NOT live-tested — Etsy app still unapproved):** Etsy access tokens expire after ~1 hour, and nothing in the codebase ever refreshed them — every Etsy read/write (Orion actions, inventory sync, flash sales, price restores, order webhook, Browse & Link) would have silently failed an hour after connecting. Added `/home/user/Tandril/supabase/functions/_shared/etsyAuth.ts` → `getEtsyAccessToken(supabase, platform)`: refreshes via `POST https://api.etsy.com/v3/public/oauth/token` (`grant_type=refresh_token`, `client_id` only — no secret needed) when within 5 min of `metadata.token_expires_at` (or when that field is missing), saves the new access **and** refresh token back to the `platforms` row, and updates the passed-in row object in place. Wired into all Etsy call sites: `smart-api` (14 places), `sync-inventory-levels`, `link-products`, `fetch-platform-products`, `fetch-product-variants`, `cancel-flash-sale`, `process-price-restores`, `process-scheduled-sales`, `etsy-order-webhook`. **Rule going forward: any new Etsy call must get its token via `getEtsyAccessToken`, never `platform.credentials.access_token` directly.** (`_shared/` imports work with the GitHub Actions auto-deploy; they would NOT work with a manual dashboard paste of a single file.)

- **Post-merge setup for PR #174 (Sept 25, 2026) — what was actually done live, and one open problem:**
  - **`ad_campaigns` / `ad_creatives` / `ad_templates` tables did not exist in production** — the Aug 11 migration (`20260811000001_ad_campaigns.sql`) had never been applied, so the Ads page and every Meta ad action had no real tables behind them (Orion's context query just returned nothing, so it failed silently). Applied it via the Supabase MCP `apply_migration` (name `ad_campaigns`), content identical to the repo file. **Lesson: migrations in `supabase/migrations/` are NOT auto-applied — only edge functions auto-deploy. After merging any PR that adds a migration, apply it by hand and verify with `select to_regclass('public.<table>')`.** (`products` is also absent, already documented as expected.)
  - **`sync-ad-performance` cron is scheduled and verified:** `cron.job` row `sync-ad-performance`, `0 */6 * * *`, created by copying `process-price-restores`'s command with only the function URL swapped (so the service-role key never had to be typed/shown). A manual run returned `200 {"success":true,"synced":0,"message":"No launched campaigns."}`.
  - **`register-shopify-webhooks` backfill FAILED for both active Shopify stores** (`omhbridge-dev.myshopify.com`, last connected July 23; `407c04-19.myshopify.com`, last connected Sept 8): every topic returned Shopify's `[API] Invalid API key or access token`. Not yet determined whether the stored tokens are genuinely revoked or only this function fails to decrypt them (it silently falls back to the still-encrypted string if `decrypt()` throws) — no other Shopify API activity was logged that day to compare against. **Simplest fix: disconnect/reconnect each Shopify store** — `shopify-auth-exchange` now registers all four order/inventory webhooks on connect, so a reconnect both refreshes the token and backfills the webhooks. ⚠️ `omhbridge-dev` is the **Shopify App Store reviewer test store** — if its token really is dead, reviewer Step 1–4 will fail; reconnect it before submitting.
  - Confirmed after deploy: `shopify-order-webhook` reaches its own HMAC check (plain `Unauthorized` on an unsigned request, not the gateway's JSON 401), and `ebay-notification-webhook` returns 200 on a public request — both `--no-verify-jwt` settings held through the auto-deploy.
- **Dead Shopify connections now detected instead of silently showing "Connected" (Sept 25, 2026, NOT yet live-tested with a revoked token):** Sarah opened Tandril after several days away — Platforms said `omamahills` (`407c04-19.myshopify.com`) was Connected, but Products was empty. Edge logs showed `get_inventory` getting a **Shopify 401** on the stored token; `get_inventory` swallowed it and returned an empty list, so nothing told her. Reconnecting fixed it (new row, all four webhooks registered, `link-products` saw 2,186 Shopify SKUs). **Root cause of the revocation not determined** — Tandril requests non-expiring offline tokens (`shopify-auth-exchange` never reads `expires_in`), so a 401 means Shopify revoked it (app uninstall/reinstall, or client-secret rotation are the likely causes); logs from when it happened had already aged out. `omhbridge-dev` (reviewer test store, user `shopify-test@tandril.org`) had the same dead token and **still needs reconnecting from that account** before any App Store submission. What was built so this can't go unnoticed again:
  - `/home/user/Tandril/supabase/functions/_shared/platformHealth.ts` — `isAuthFailure(err)` (401/403/"Invalid API key or access token"), `markNeedsReconnect(supabase, platform, reason)` sets `platforms.status = 'needs_reconnect'` + `metadata.connection_error`, `markHealthy()` reverts. `is_active` stays true so every existing `.or('is_active.eq.true,status.eq.connected')` query still finds the row.
  - New allowed status value: migration `20260925000002_platform_needs_reconnect_status.sql` widened the `platforms_status_check` constraint — **already applied live** via MCP the same day.
  - Flag is set by: `smart-api` `get_inventory` (also now returns `connection_issues`) and `getUserStoreContext`'s Shopify fetch; `sync-inventory-levels` on any auth failure pushing to a target platform; `app-uninstalled` (flags with `reason: 'uninstalled'` and emails immediately — previously an uninstall just nulled the token and set `is_active=false` silently); and the new **hourly `check-platform-connections`** function (service-role only), which (1) pings every active Shopify store with `{ shop { name } }` and on first detection drops a high-priority `smart_alerts` row, (2) for healthy stores lists `webhookSubscriptions` and re-registers any of `ORDERS_PAID`/`ORDERS_CANCELLED`/`REFUNDS_CREATE`/`INVENTORY_LEVELS_UPDATE` pointing at `shopify-order-webhook` that are missing (Shopify deletes a subscription after repeated delivery failures — that would silently stop instant sync with the store still "Connected"), and (3) emails sellers about every `needs_reconnect` row of any platform. Non-auth errors (Shopify outage) don't flag.
  - **Seller emails** (`/home/user/Tandril/supabase/functions/_shared/connectionAlerts.ts`, from `Tandril Alerts <alerts@tandril.org>` via Resend): first email as soon as the hourly job sees the flag (instant for uninstalls), reminders at 24h and 72h, then stop; an uninstall gets only the one email. Progress tracked in `metadata.connection_error.{at, reason, emails_sent, last_emailed_at}`; `markNeedsReconnect` preserves `at`/`emails_sent` when re-flagging so repeated failures don't restart the schedule. Uses `webhookSubscriptions.endpoint.callbackUrl`, valid on Admin API `2025-01` (deprecated in later versions in favour of `uri` — update if the API version is bumped).
  - Cleared by: all four Shopify connect paths (`shopify-auth-exchange`, `-callback`, `-manual-connect`, `-app-install`) now set `status: 'connected'` in their upsert — previously a reconnect of an existing row never touched `status`.
  - UI: `PlatformCard.jsx` shows an amber "Reconnect needed" badge + explanation + a one-click **Reconnect Store** button (Shopify) instead of "Connected"; new `components/platforms/ReconnectBanner.jsx` shown on `pages/Products.jsx` and `pages/Inventory.jsx`; Orion's system prompt gets a `reconnectSection` telling it to lead with the problem instead of reasoning from missing products. `smart-api`'s Shopify platform lookup in `executeStoreAction` now orders by `updated_at desc` (was a bare `limit(1)`).
  - **`check-platform-connections` cron is scheduled and verified live (Sept 25, 2026, after PR #175 merged):** `cron.job` `check-platform-connections`, `7 * * * *`, created by copying `process-price-restores`' command with the URL swapped. First manual run returned `200 {"checked":2,"needs_reconnect":1,"emails_sent":1}` — it flagged `omhbridge-dev` (dead token), created its bell alert, and Resend accepted the reconnect email to that account (`shopify-test@tandril.org`); `407c04-19` (omamahills, reconnected same day) came back healthy with all four sync webhooks already registered (`webhooks_repaired: []`). ~~Still to do: reconnect `omhbridge-dev` from the `shopify-test@tandril.org` Tandril login~~ — done Sept 25, verified healthy.
  - **Planned next (discussed with Sarah Sept 25, not built):** (a) automatic catch-up sync when a store reconnects — set every linked SKU to the lowest quantity across platforms, since sales during the outage were never propagated; (b) a 15–30 min background reconciliation sweep comparing linked SKUs across platforms and fixing drift with the same lowest-wins rule, as the backstop that doesn't depend on webhooks; (c) optional per-seller safety-stock buffer.
  - Not covered yet: non-Shopify platforms aren't pinged by the daily check (eBay/Etsy self-refresh; others only get flagged when a real call fails).
- **WooCommerce + Etsy order webhooks were blocked by the gateway, and the hourly connection check now covers every syncing platform (Sept 25, 2026, NOT live-tested yet):**
  - **Same bug class as `shopify-order-webhook` (Sept 5) and `ebay-notification-webhook` (Sept 25):** `woocommerce-order-webhook` and `etsy-order-webhook` were missing from the deploy workflow's `--no-verify-jwt` list. Confirmed live: a public POST to either returned Supabase's `{"code":"UNAUTHORIZED_NO_AUTH_HEADER"}` before the function ran, so **WooCommerce and Etsy sales never auto-synced**. Added both to `.github/workflows/deploy-edge-functions.yml`. Safe to open: both re-read the real order/stock from the platform API instead of trusting the payload (a forged call can only trigger a re-sync of real numbers); WooCommerce also verifies its HMAC when `metadata.webhook_secret` is stored. `etsy-order-webhook` now also verifies a Standard Webhooks signature (`webhook-id`/`webhook-timestamp`/`webhook-signature`, 5-min replay window) **if** a new optional `ETSY_WEBHOOK_SIGNING_SECRET` Supabase secret is set — set it from the Etsy app's webhook settings once Etsy approves the app; the exact Etsy signing scheme is unverified (Etsy docs weren't reachable).
  - **WooCommerce auto-disables a webhook after repeated delivery failures**, so existing WooCommerce stores' Tandril webhooks were likely disabled by those 401s. `check-platform-connections` now re-activates (`PUT status: active`) or re-creates them hourly.
  - **`check-platform-connections` now checks Shopify, WooCommerce, eBay and Etsy** (the four platforms `sync-inventory-levels` can write to), each with its own checker: Shopify `{ shop { name } }` + webhook repair (unchanged); WooCommerce `GET /wp-json/wc/v3/webhooks` (login check) + webhook re-activate/re-create; eBay refresh-if-expiring (a refresh `invalid_grant` = dead refresh token → flag) + `GET apiz.ebay.com/commerce/identity/v1/user/`; Etsy `getEtsyAccessToken` + `GET /v3/application/shops/{shop_id}`. Auth failures (401/403, `invalid_grant`, missing credentials) flag `needs_reconnect` + bell alert + the existing email/reminder flow; anything else (site down, outage) just retries next hour. `PLATFORM_LABELS` is now exported from `_shared/connectionAlerts.ts`.
  - ⚠️ **eBay instant sale sync is probably not working at all — unverified, needs checking before anyone relies on it.** `ebay-auth-callback` subscribes to Commerce Notification topic `marketplace.ORDER` with a user token and a minimal destination/subscription body. That topic id doesn't appear in eBay's documented topics (search results show e.g. `ORDER_CONFIRMATION`, `ITEM_MARKED_SHIPPED`, which are buyer-side), destinations normally require an application token plus `name`/`status`, and subscriptions normally require `destinationId`/`status`/`payload`. Seller "item sold" events have historically come from Trading API Platform Notifications (`SetNotificationPreferences`, e.g. `FixedPriceTransaction`), not the Commerce Notification API. Verify with a real eBay sale and `GET /commerce/notification/v1/subscription` before trusting eBay instant sync; the planned 15–30 min reconciliation sweep is the dependable eBay path either way.
  - **Verified live after PR #177 merged (Sept 25, 2026):** public POSTs to `woocommerce-order-webhook` and `etsy-order-webhook` now return the function's own `ok` instead of the gateway 401. A manual `check-platform-connections` run returned `200 {"checked":4,"needs_reconnect":1,"emails_sent":0}`: both eBay rows ok (`saraevens` on omamahills@gmail.com, plus an older May 7 "eBay - eBay User" row on Sarah's second account evensonsarah704@gmail.com — a separate account, not a duplicate), omamahills Shopify ok, `omhbridge-dev` still flagged (already emailed, so no repeat email yet — the 24h reminder is next). No WooCommerce/Etsy stores are connected in production yet, so those two checkers have not run against a real store. **Later the same day Sarah reconnected both Shopify stores to the right accounts** — `407c04-19` (O'Mama Hill's) on omamahills@gmail.com and `omhbridge-dev` on `shopify-test@tandril.org` (the reviewer account; the store must live there, not on Sarah's own account, or the reviewer sees nothing). Re-run returned `{"checked":4,"needs_reconnect":0}` — all four connections healthy, all Shopify sync webhooks present. The App Store reviewer store is no longer blocked.
  - Not built yet: expiry warnings before eBay (~18-month refresh token) / Meta (~60-day token) logins lapse; the reconciliation sweep.
- **Disconnecting a store no longer deletes its product links — they're saved, paused, and restored on reconnect (Sept 25, 2026):** `platform_product_links.platform_id` is `ON DELETE CASCADE`, so the Disconnect button (which deletes the `platforms` row) silently wiped every link for that store — including when a seller disconnected only to reconnect, which is exactly what the reconnect emails tell them to do. Found live: Sarah disconnected/reconnected O'Mama Hill's and the Shopify half of her one linked product (Halloween tote, SKU `1671288_4533`) vanished, leaving an eBay-only link. What changed:
  - Migration `20260925000004_pause_links_on_disconnect.sql`: new `paused_product_links` table (RLS: users select/delete own; deliberately **no FK to auth.users** — the pause trigger fires during account deletion and an FK would block it). `platform_store_key(platforms)` = lowercased `shop_domain` → `store_url` → `metadata.shop_id` → `name`. **BEFORE DELETE trigger** on `platforms` copies the store's links into `paused_product_links` (skipped when the account itself is being deleted); the cascade still removes the live links so nothing syncs to a disconnected store. **AFTER INSERT/UPDATE trigger** restores links for the same user + platform type + store key when the row is active/connected, and sets `platforms.metadata.links_restored = {count, skus, at, pending: true}`. A different store never matches. Tested in a rolled-back transaction against production (save 2 → other store restores 0 → same store restores 2 with variant ids and last quantities intact → flag set).
  - Catch-up: `sync-inventory-levels` has a new `mode: 'lowest'` — reads current stock on every linked platform and pushes the **lowest** to the rest (sales made while disconnected never reached the store; lowest can't oversell). `pages/Platforms.jsx` runs it for each restored SKU as soon as it sees `links_restored.pending`, toasts "Restored N product links for X", and clears `pending`. `check-platform-connections` does the same hourly as a backstop (waits 10 min so it doesn't race the page; leaves `pending` set if any SKU failed).
  - Disconnect dialog now counts the store's links and says exactly: the store is disconnected, nothing syncs while it's disconnected, the N links are **saved and paused, not deleted**, and they're restored automatically on reconnecting **this same store** with stock brought back in line (lowest kept); a different store won't restore them. `SyncLinksPanel.jsx` shows a "N product links saved and paused for X" notice per disconnected store.
  - `shop-redact` also deletes the shop's `paused_product_links` (GDPR delete-for-good).
  - **Migration must be applied by hand after merge** (see the migrations lesson above). Links deleted before this change (like the tote's Shopify half) are gone — re-link them once.
...
| Walmart Marketplace | Fixed Sept 20, 2026 — was the same "0 products on Products/Inventory page" bug class as WooCommerce/BigCommerce, self-service `client_id`+`client_secret` (no OAuth review), NOT yet live-tested (see Key Fixes) |
...
- **Manual inventory edit in Shopify admin** (typing a new stock number) → **now auto-syncs (added Sept 25, 2026, NOT yet live-tested)** via a new `inventory_levels/update` webhook handled in `shopify-order-webhook`: it looks up the inventory item's SKU and total available across locations, and only syncs if that SKU is linked and the number differs from the link's `last_synced_quantity` — that check stops Tandril's own writes to Shopify from echoing back out, and stops the same sale from syncing twice (orders/paid + inventory_levels/update both fire). `sync-inventory-levels` now also stamps `last_synced_quantity` on the **source** link, not just targets, so the check works. All five Shopify connect paths (`shopify-auth-exchange`, `shopify-auth-callback`, `shopify-app-install`, `shopify-manual-connect`, `register-shopify-webhooks`) now register `orders/paid` + `orders/cancelled` + `refunds/create` + `inventory_levels/update` — previously three of them registered only `orders/paid`. **Stores connected before this change don't have the new webhooks** — after merge, call `register-shopify-webhooks` once with the service-role key (or reconnect the store) to backfill them. Before that, the old behaviour below still applies to those stores: The only way to push a manual edit to linked platforms is clicking **Sync Now** (per-SKU) or **Reconcile All** in the Inventory page's "Cross-Platform Sync Links" panel (`/home/user/Tandril/components/inventory/SyncLinksPanel.jsx`, rendered directly on `pages/Inventory.jsx`). The 15-min `pg_cron` retry job (`process-sync-retries`) only retries previously *failed* syncs — it does not detect new manual changes, so there is no passive fallback that eventually catches a manual edit.
...
- ~~`shopify-order-webhook` has no active HMAC verification~~ — fixed Sept 5, 2026 (PR #170), now verifies against `SHOPIFY_API_SECRET`. Also (Sept 25) its platform lookup no longer uses a bare `.single()` — it picks the most recently updated active row, per the shop-identity lesson in Key Fixes.
...
- Edge functions auto-deploy from GitHub on merge to main (see "Supabase Edge Functions" above) — manual dashboard pasting is a fallback, not the normal path
...
- ~~`ad_account_id` is a manually-typed field~~ — **fixed Sept 25, 2026:** `launchMetaCampaign` now calls `resolveMetaAdAccountId()` when no `ad_account_id` is given: uses `platforms.metadata.ad_account_id` if saved, else looks up `me/adaccounts`, and if exactly one is active, uses it and saves it to metadata. If several are active it errors with the list (never guesses where to spend money). The field in `CreateCampaignModal` is now optional. Works on existing connections — no reconnect needed.
- ~~No natural-language Orion integration~~ — **built Sept 25, 2026, NOT live-tested:** `chatWithClaude` now includes an `adsSection` in Orion's system prompt (only when not in demo mode): Meta connection status (expired token / missing Page warnings), the 10 most recent `ad_campaigns` with id/status/budget/cached metrics (loaded in `getUserStoreContext` as `meta_ads` + `ad_campaigns`), the four action schemas, and rules — **default to `draft_ad`; `launch_ad` only when the user explicitly says launch/go live, with a spend warning in the same message; never invent a budget or URL; never `CONVERSIONS`; one ad block per response.** Orion can pass `product_handle` (a Shopify handle) and `enrichAdActionFromProduct()` fills `creative.link` (the product's `onlineStoreUrl`, falling back to `https://{shop}.myshopify.com/products/{handle}`), `creative.media_urls[0]` (featured image) and `linked_product_id` via Shopify GraphQL. `AIBusinessCoach.jsx` has dedicated confirmation cards for all four (launch card shows a ⚠️ spending line). Non-Shopify products still need the user to give a URL.

...
serve(async (req) => {
  ...
  try {
    ...
    if (fetchErr) throw new Error(`Failed to fetch pending restores: ${fetchErr.message}`);
    if (!pending || pending.length === 0) {
      return new Response(JSON.stringify({ success: true, restored: 0, message: 'No restores due.' }), {
    ...
    for (const row of pending) {
      try {
        ...
        console.error(`[process-price-restores] Failed row ${row.id}:`, err.message);
        await supabase.from('scheduled_restores')
    ...
    return new Response(JSON.stringify({ success: true, restored, failed }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });

  } catch (err) {
    console.error('[process-price-restores]', err.message);
  ...
  switch (row.restore_type) {
    ...
    case 'price': {
      ...
      if (row.platform_type === 'etsy') {
        ...
        if (!res.ok) throw new Error(`Etsy restore failed: ${res.status} ${await res.text()}`);
        break;
      }

      throw new Error(`Unknown platform for restore_type=price: ${row.platform_type}`);
    }

    case 'woo_sale_price': {
      let token = plat.access_token;
      const ck = token ? token.split(':')[0] : plat.credentials?.consumer_key;
      const cs = token ? token.split(':')[1] : plat.credentials?.consumer_secret;
      ...
      const res = await fetch(`${wooBase}/wp-json/wc/v3/products/${row.platform_product_id}`, {
        ...
        body: JSON.stringify({ sale_price: '' }),  // empty string clears sale price in WooCommerce
      });
      if (!res.ok) throw new Error(`WooCommerce restore failed: ${res.status} ${await res.text()}`);
      break;
    }

    case 'end_promotion': {
      if (row.platform_type !== 'ebay') throw new Error('end_promotion only supported for eBay');
      ...
      if (!meta.token_expires_at || Date.now() > new Date(meta.token_expires_at).getTime() - 5 * 60 * 1000) {
        const ck = Deno.env.get('EBAY_CLIENT_ID');
        const cs = Deno.env.get('EBAY_CLIENT_SECRET');
      ...
      // 204 No Content = success; 404 = already ended (treat as success)
      if (!res.ok && res.status !== 404) throw new Error(`eBay end promotion failed: ${res.status} ${await res.text()}`);
      break;
    }

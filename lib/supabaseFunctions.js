...
/**
 * Same as getShopifyInventory, plus the stores whose login the platform rejected
 * while loading (e.g. a Shopify store whose access was revoked), so the page can
 * say "reconnect" instead of just showing an empty list.
 * @returns {Promise<{inventory: Array, connectionIssues: Array<{platform_type, platform_id, shop}>}>}
 */
export async function getInventoryWithStatus() {
  const response = await invokeEdgeFunction('smart-api', {
    execute_action: { type: 'get_inventory' },
  });
  return {
    inventory: response?.execution_result?.inventory || [],
    connectionIssues: response?.execution_result?.connection_issues || [],
  };
}

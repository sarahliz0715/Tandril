...
export async function invokeEdgeFunction(functionName, params = {}) {
  ...
  try {
    console.log(`[Supabase Functions] Invoking ${functionName} with params:`, params);

    const { data, error } = await supabase.functions.invoke(functionName, {
      body: params,
    });

    console.log(`[Supabase Functions] Raw response from ${functionName}:`, { data, error });
    ...
    if (error) {
      ...
      let errorMessage = error.message || `Function ${functionName} failed`;
      if (error.context) {
        try {
      ...
      console.error(`[Supabase Functions] Actual error:`, errorMessage);
      throw new Error(errorMessage);
    }

    console.log(`[Supabase Functions] Data from ${functionName}:`, data);
    console.log(`[Supabase Functions] Data type:`, typeof data);

    // Edge functions return {success, data, error} format
    if (data && !data.success && data.error) {
      console.error(`[Supabase Functions] Function ${functionName} returned error:`, data.error);
    ...
    console.error(`[Supabase Functions] Exception type:`, error.constructor.name);
    console.error(`[Supabase Functions] Exception message:`, error.message);
    throw error;
  }
}
...
/**
 * Run Reorder Point Calculator
 * Deterministic (no AI) — calculates sales velocity, days of stock remaining, and a
 * reorder point per SKU using linked supplier lead times. Products with no linked
 * supplier are skipped, not defaulted. Fires smart_alerts for SKUs that need reordering.
 * @param {object} options - Configuration options
 * @param {number} options.trailing_window_days - Sales velocity lookback window (default: 30)
 * @param {number} options.safety_buffer_percent - Extra buffer on lead-time demand (default: 20)
 * @param {number} options.alert_cooldown_hours - Dedupe window for repeat alerts (default: 24)
 * @param {string} options.workflow_id - Optional workflow ID for tracking
 * @returns {Promise<{needs_reorder_count: number, skipped_count: number, alerts_created: number, results: Array}>}
 */
export async function runReorderPointCalculator({
  trailing_window_days = 30,
  safety_buffer_percent = 20,
  alert_cooldown_hours = 24,
  workflow_id = null
} = {}) {
  const response = await invokeEdgeFunction('reorder-point-calculator', {
...
export async function getShopifyInventory() {
  const response = await invokeEdgeFunction('smart-api', {
    execute_action: { type: 'get_inventory' },
  });
  return response?.execution_result?.inventory || [];
}
...
export async function chatWithCoach({
  ...
  // invokeEdgeFunction returns the response directly (not wrapped in data)
  return response;
}
...
/**
 * Complete the Shopify OAuth exchange (called from frontend after the Vercel proxy redirect)
 * @param {object} params
 * @param {string} params.code - Authorization code from Shopify
 * @param {string} params.state - State parameter for CSRF validation
 * @param {string} params.shop - Shop domain (e.g. my-store.myshopify.com)
 */
export async function shopifyAuthExchange({ code, state, shop } = {}) {
  const response = await invokeEdgeFunction('shopify-auth-exchange', { code, state, shop });
  return response;
}

/**
 * Connect Shopify using a manual Admin API access token instead of OAuth.
 *
 * This is an alternate path for stores whose OAuth install is blocked by
 * Shopify's app-review gate ("This app is under review"). The merchant
 * creates a "custom app" directly in their own Shopify admin (Settings >
 * Apps and sales channels > Develop apps), which issues an Admin API
 * access token with no Shopify review required, and pastes that token in.
 * Storage shape on the backend is identical to the OAuth path, so nothing
 * else in the app needs to know which method was used to connect.
 *
 * @param {object} params
 * @param {string} params.shop_domain - e.g. "my-store" or "my-store.myshopify.com"
 * @param {string} params.access_token - Admin API access token (e.g. shpat_...)
 */
export async function connectShopifyManual({ shop_domain, access_token } = {}) {
  const response = await invokeEdgeFunction('shopify-manual-connect', { shop_domain, access_token });
  return response;
}
...
// ─── New unified OAuth functions ───────────────────────────────────────────────

export async function initiateOAuth({ platform } = {}) {
  const response = await invokeEdgeFunction('oauth-init', { platform });
  return response;
}

export async function handleOAuthCallback({ code, state } = {}) {
  const response = await invokeEdgeFunction('oauth-callback', { code, state });
  return response;
}

export async function connectWalmart({ client_id, client_secret } = {}) {
  const response = await invokeEdgeFunction('walmart-connect', { client_id, client_secret });
  return response;
}

export async function connectEcwid({ store_id, access_token } = {}) {
  const response = await invokeEdgeFunction('ecwid-connect', { store_id, access_token });
  return response;
}

export async function connectMagento({ store_url, access_token } = {}) {
  const response = await invokeEdgeFunction('magento-connect', { store_url, access_token });
  return response;
}

export async function connectPrestaShop({ store_url, api_key } = {}) {
  const response = await invokeEdgeFunction('prestashop-connect', { store_url, api_key });
  return response;
}

export async function connectWish({ access_token } = {}) {
  const response = await invokeEdgeFunction('wish-connect', { access_token });
  return response;
}

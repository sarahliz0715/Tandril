...
export const supabaseAuthService = {
  ...
  /**
   * Get current user with fresh metadata from Supabase.
   * Falls back to cached session if getUser() fails (e.g. during OAuth redirect).
   */
  async me() {
    // getUser() hits the Supabase server and returns up-to-date user_metadata,
    // so plan changes made via webhooks are visible immediately without re-login.
    const { data: { user: freshUser } } = await supabase.auth.getUser().catch(() => ({ data: { user: null } }));

    // Fall back to cached session if getUser() fails (e.g. during OAuth redirect)
    let user = freshUser;
    if (!user) {
    ...
    return {
      ...
      platforms_limit: user.user_metadata?.platforms_limit || 2,
      api_usage_limit: user.user_metadata?.api_usage_limit || 50,

      // Onboarding
      onboarding_completed: user.user_metadata?.onboarding_completed || false,

      // Admin
      isAdmin: user.user_metadata?.isAdmin || user.email === 'sarahliz0715@gmail.com' || user.email === 'omamahills@gmail.com',
      role: user.user_metadata?.role || (['sarahliz0715@gmail.com', 'omamahills@gmail.com'].includes(user.email) ? 'owner' : 'user'),
...
export async function syncShopifyPlan() {
  try {
    ...
    const res = await fetch('/api/shopify-sync-plan', {
      method: 'POST',

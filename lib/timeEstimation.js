...
/**
 * Calculate minutes saved for a single completed command.
 * @param {object} command - An ai_commands record
 * @returns {number} Minutes saved (always >= baseMinutes)
 */
export function minutesForCommand(command) {
  ...
  // success_count lives in execution_results or results (depending on source)
  const itemsAffected =

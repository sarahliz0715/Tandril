...
export class EbayAdapter extends PlatformAdapter {
  ...
  async verifyWebhookSignature(payload, signature) {
    const signingKey = this.config?.signingKey ||
      (typeof process !== 'undefined' ? process.env?.EBAY_SIGNING_KEY : null);

    if (!signingKey) {
      console.warn('[EbayAdapter] No signing key configured — set signingKey in adapter config or EBAY_SIGNING_KEY env var');
      return false;
    }

    if (!signature) {
      console.warn('[EbayAdapter] Missing signature header');
      return false;
    }
    ...
    try {
      ...
      console.error('[EbayAdapter] Webhook signature verification error:', err.message);
      return false;
    }
  }
